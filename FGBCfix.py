...
import math
import numpy as np
import wideresnetwithFGBC as models
import torch
...
if args.dataset=='cifar10':
    ...
    import dataset.stl10 as dataset
    print(f'==> Preparing imbalanced STL10')
    num_class = 10
elif args.dataset=='smallimagenet127':
    # import dataset.imbSmallImageNet127 as dataset
    import dataset.fix_small_imagenet127 as dataset
    print(f'==> Preparing smallimagenet127')
    num_class = 127
# Use CUDA
...
def main():
    ...
    if args.dataset == 'cifar10':
        ...
        train_labeled_set, train_unlabeled_set,test_set, _, _, _ = dataset.get_stl10('./data', N_SAMPLES_PER_CLASS,U_SAMPLES_PER_CLASS)
    elif args.dataset == 'svhn':
        ...
        img_size2path = {32: '/home/weilegexiang/980SSD/DataSets/ImageNet127/x32', 64: '/home/weilegexiang/980SSD/DataSets/ImageNet127/x64'}
        N_SAMPLES_PER_CLASS, train_labeled_set, train_unlabeled_set, test_set = dataset.get_small_imagenet(img_size2path[args.img_size], args.img_size, labeled_percent=0.01 * args.label_ratio, seed=args.manualSeed)
        ir3 = (sum(N_SAMPLES_PER_CLASS)/num_class)/np.array(N_SAMPLES_PER_CLASS)
        ir2 = min(N_SAMPLES_PER_CLASS)/np.array(N_SAMPLES_PER_CLASS)
        print('N_SAMPLES_PER_CLASS', N_SAMPLES_PER_CLASS, ir2, ir3)
    ...
    # Model
    print("==> creating Resnet50 with FGBC")
    ...
    if args.resume:
        ...
        logger.set_names(['Train Loss', 'Train Loss X', 'Train Loss U', 'abcloss','Test Loss', 'Test Acc.', 'Minority Acc.'])
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.dataset == 'smallimagenet127':
            test_loss, test_acc, test_cls, test_gm, per_cls_acc = validate_avgclass(test_loader, ema_model, criterion, use_cuda, mode='Test Stats ')
        else:
        ...
        if args.dataset == 'cifar10':
            ...
            print("each class accuracy test", testclassacc, testclassacc.mean(), testclassacc[:50].mean(),testclassacc[5:].mean())
        elif args.dataset == 'smallimagenet127':
            print('test_loss, per_cls_acc.mean().tolist(), test_acc', per_cls_acc, test_loss, per_cls_acc.mean().tolist(), test_acc, per_cls_acc[int(num_class/2):].mean()*100.)
            
        logger.append([train_loss, train_loss_x, train_loss_u,abcloss, test_loss, test_acc, testclassacc[5:].mean()*100.])
...
def getDiffRankWeight(logit1, logit2):
    diff_u = torch.sum(torch.square(logit1-logit2), dim=1)
    diff_u_sort, sort_idx = torch.sort(diff_u)
    sort_idx = sort_idx.cpu().tolist()
    diff_rank_weight = [0] * len(sort_idx)
    for rank, idx in enumerate(sort_idx):
        diff_rank_weight[idx] = 1 + 0.5 * 1 * (1 + math.cos(rank*1.0*math.pi/(len(sort_idx)-1)))
    diff_rank_weight = torch.tensor(diff_rank_weight).cuda().detach()
    return diff_rank_weight
...
def train(labeled_trainloader, unlabeled_trainloader, model, optimizer, ema_optimizer, criterion, epoch,ir2):
    ...
    for batch_idx in range(args.val_iteration):
        ...
        basethed = 0.5 #+ (args.label_ratio /100.0)*0.5

        if args.dataset == 'cifar100':
            #p_cutoff = min([basethed + math.sin(math.pi*epoch / (args.epochs*2))*(1.-basethed),0.999])  #EASY
            p_cutoff = min([1.0 - math.sin(math.pi*epoch / (args.epochs*2))*0.4,1.0])  #HARD 0.2 for 10
            #p_cutoff = min([0.8 + 0.2*epoch / args.epochs,1.0])  #EASY
        else:
            #p_cutoff = min([basethed + math.sin(math.pi*epoch / (args.epochs*2))*(1.-basethed),0.999])  #HARD
            #p_cutoff = 0.99
            p_cutoff = min([1.0 - math.sin(math.pi*epoch / (args.epochs*2))*0.2,1.0])  #HARD 0.2 for 10

        beta_ = ir2[::-1]
        beta = torch.tensor(beta_.copy()).cuda(0)
        max_p, p_hat = torch.max(targets_u2, dim=1)
        #print(beta)
        #imb_fac_base = 10.0 - args.imb_ratio/100.0
        imb_fac_base = 9.0
        #print(imb_fac_base)
        select_mask = max_p.ge(p_cutoff *(beta[p_hat]*(10.0-imb_fac_base) + imb_fac_base) / 10.0).float()
        ...
        logits_x, rn_weight_x=model.classify(q, cal_rn_weight=True)
        ...
        logit, before_logit, rn_weight_l = model.classify2(q, cal_rn_weight=True)
        logitu1, before_logitu1, _ = model.classify2(q1, cal_rn_weight=False)
        logitu2, before_logitu2, _ = model.classify2(q2, cal_rn_weight=False)
        logitu3, before_logitu3, _ = model.classify2(q3, cal_rn_weight=False)
 


        if epoch >400:
            base_thdd = 0.5# +(epoch / args.epochs)*0.5
            #base_thdd = 1.0 - min(0.5, 0.5*args.imb_ratio/100.0)
            thdd = base_thdd + (beta_[targets_x])*(1.0-base_thdd)
            l = np.random.uniform(thdd,1)
            
            l = torch.tensor(l.copy()).cuda(0)
            l = l.reshape(batch_size,1).cuda().float()
            mixed_q = l * q + (1 - l) * q1#.cuda(0).detach()
            mixed_input, before_logit, rn_weight_l = model.classify2(mixed_q, cal_rn_weight=True)
            logits = F.softmax(mixed_input, dim=1)
            #logits = F.softmax(mixed_input+torch.log(beta), dim=1)
        else:
            logits = F.softmax(logit)
            #logits = F.softmax(logit+torch.log(beta))

        logitsu1 = F.softmax(logitu1)
        logitsu2 = F.softmax(logitu2)
        logitsu3 = F.softmax(logitu3)
        
        before_logitu1 = F.softmax(before_logitu1, dim=1)
        before_logitu2 = F.softmax(before_logitu2, dim=1)
        before_logitu3 = F.softmax(before_logitu3, dim=1)
        before_logit = F.softmax(before_logit, dim=1)

        diff_rank_weight_u = getDiffRankWeight(logitsu2, logitsu3)


        max_p2, label_u = torch.max(logitsu1, dim=1)
        #select_mask2 = max_p2.ge(0.95)
        select_mask2 = max_p2.ge(p_cutoff *(beta[label_u]*(10.0-imb_fac_base) + imb_fac_base) / 10.0).float()
        ...
        abcloss = -torch.mean(maskforbalance * torch.sum(torch.log(torch.clamp(logits,1e-12,1.0)) * targets_x2.cuda(), dim=1))
        abcloss1 = -torch.mean(  
            select_mask2 * maskforbalanceu * torch.sum(torch.log(torch.clamp(logitsu2,1e-12,1.0)) * logitsu1.cuda().detach(), dim=1))
        abcloss2 = -torch.mean(  
            select_mask2 * maskforbalanceu * torch.sum(torch.log(torch.clamp(logitsu3,1e-12,1.0)) * logitsu1.cuda().detach(), dim=1))
        ...
        Lx, Lu = criterion(logits_x, all_targets[:batch_size], logits_u, all_targets[batch_size:], select_mask, rn_weight_x)
...
def validate_avgclass(valloader, model, criterion, use_cuda, mode):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets, _) in enumerate(valloader):
            ...
            targetsonehot = torch.zeros(inputs.size()[0], num_class).scatter_(1, targets.cpu().view(-1, 1).long(), 1)
            q=model(inputs)
            outputs=model.classify2(q)

            unbiasedscore = F.softmax(outputs)
...
class SemiLoss(object):
    def __call__(self, outputs_x, targets_x, outputs_u, targets_u, mask, rn_weight=None,rn_weight_lu=None):

...
def main():
    ...
    ir2=np.clip(N_SAMPLES_PER_CLASS[-1]/np.array(N_SAMPLES_PER_CLASS),1.0/args.batch_size,1.0)
    if args.dataset == 'cifar10':
    ...
    print("==> creating WRN-28-2 with FGBC")
    ...
    for epoch in range(start_epoch, args.epochs): # for svhn,  range(start_epoch, args.epochs-50)
...
def getDiffRankWeight(logit1, logit2):
    ...
    diff_u_sort, sort_idx = torch.sort(diff_u, descending = True)
    sort_idx = sort_idx.cpu().tolist()
    diff_rank_weight = [0] * len(sort_idx)
    for rank, idx in enumerate(sort_idx):
        diff_rank_weight[idx] = 0.75 + 0.5 * 0.5 * (1 + math.cos(rank*1.0*math.pi/(len(sort_idx)-1)))
        #diff_rank_weight[idx] = 0.5 * 1.0 * (1 + math.cos(rank*1.0*math.pi/(len(sort_idx)-1)))
...
def train(labeled_trainloader, unlabeled_trainloader, model, optimizer, ema_optimizer, criterion, epoch,ir2, emp_distb_u,target_disb):
    ...
    for batch_idx in range(args.val_iteration):
        ...
        #mixed_target[:batch_size] = target_a[:batch_size]
        ...
        if args.dataset!='cifar100' and args.imbalancetype=='step':
            if args.dataset =='cifar100':
                maskforbalance = torch.sum(targets_x2 * torch.tensor(ir22).cuda(0),dim=1).detach()
            else: 
                maskforbalance = torch.bernoulli(torch.sum(targets_x2 * torch.tensor(ir22).cuda(0),dim=1).detach())
        else:
            if args.dataset =='cifar100':
                maskforbalance = torch.sum(targets_x2 * torch.tensor(ir2).cuda(0), dim=1).detach()
            else : 
        ...
        #imbmix = np.sqrt(beta_)
        #print(beta_)

        if epoch >400:
            base_thdd = 0.5 #+(epoch / args.epochs)*0.5
            #base_thdd = 0.5 + np.clip(( args.label_ratio/args.imb_ratio) *0.5,0.0, 0.5) #+(epoch / args.epochs)*0.5
        ...
        #p_cutoff = min([1.0 - math.sin(math.pi*epoch / (args.epochs*2))*0.1,1.0])  #HARD
        #
        p_cutoff = max([1.0 - math.sin(math.pi*epoch / (args.epochs*2))*0.1,0.95])  #HARD
        if args.dataset =='cifar100':
            p_cutoff = max([1.0 - math.sin(math.pi*epoch / (args.epochs*2))*0.2,0.90])  #HARD
        #p_cutoff = 0.95  #HARD
        max_p, label_u = torch.max(logitsu1, dim=1)
        #beta2 = 1 - ((args.epochs - epoch) / args.epochs) * (1 - beta)

        #print(beta2)
        if epoch >400:
            #p_cutoff = min([0.5 + math.sin(math.pi*epoch / (args.epochs*2))*0.5,0.99])  #HARD
            #p_cutoff = 0.99
            if args.dataset =='cifar100':
                p_cutoff = 0.99
                #select_mask = max_p.ge(p_cutoff)
            #else:
            basethed = 10.0 #- min(((args.label_ratio /10.0)-1),1.0)
            select_mask = max_p.ge(p_cutoff *(beta[label_u]*(10.0-basethed) + basethed) / 10.).float()
        else:             
            select_mask = max_p.ge(p_cutoff *(beta[label_u] + 9.) / 10.).float()

        label_u = torch.zeros(batch_size, num_class).scatter_(1, label_u.cpu().view(-1, 1), 1)
        if args.dataset =='cifar100':
            maskforbalanceu = torch.sum(label_u.cuda(0) * torch.tensor(ir22).cuda(0), dim=1).detach()
        else: 
       ...
       # maskforbalanceu = torch.bernoulli(torch.sum(label_u.cuda(0) * torch.tensor(ir2).cuda(0), dim=1).detach())
        ...
        #logitsu2 = F.softmax(logitu2+torch.log(beta))
        #logitsu3 = F.softmax(logitu3+torch.log(beta))

        #diff_rank_weight_u = getDiffRankWeight(logitsu2, logitsu3)

        abcloss = -torch.mean(maskforbalance * torch.sum(torch.log(torch.clamp(logits,1e-12,1.0)) * targets_x2.cuda(0), dim=1))
        abcloss1 = -torch.mean( 
            select_mask * maskforbalanceu * torch.sum(torch.log(torch.clamp(logitsu2,1e-12,1.0)) * logitsu1.cuda(0).detach(), dim=1))

        abcloss2 = -torch.mean(
            select_mask * maskforbalanceu * torch.sum(torch.log(torch.clamp(logitsu3,1e-12,1.0)) * logitsu1.cuda(0).detach(), dim=1))


        #abcloss = -torch.mean(torch.sum(torch.log(torch.clamp(logits,1e-8,1.0)) * targets_x2.cuda(0), dim=1))
        #abcloss1 = -torch.mean( 
        #    select_mask * torch.sum(torch.log(torch.clamp(logitsu2,1e-8,1.0)) * logitsu1.cuda(0).detach(), dim=1))

        #abcloss2 = -torch.mean(
        #    select_mask * torch.sum(torch.log(torch.clamp(logitsu3,1e-8,1.0)) * logitsu1.cuda(0).detach(), dim=1))
